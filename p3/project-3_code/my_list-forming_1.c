...
int K; // genreate a data node for K times in each thread
...
void * producer_thread( void *arg )
{
	...
	Local_List = (struct list *)malloc(sizeof(struct list));
	if( NULL == Local_List )
    {
       printf("End here\n");
       exit(0);	
	...
	while(1)
	{
		...
		{
			/* attach the generated local list to the global list */
			if( List->header == NULL )
			{
...
int main(int argc, char* argv[])
{
    ...
    if(argc < 3){
        printf("ERROR: please provide two input arg (the number of threads and K number of operations per thread)\n");
        exit(1);
    }

    num_threads = atoi(argv[1]); //read num_threads from user
	K = atoi(argv[2]); // genreate a data node for K times in each thread
